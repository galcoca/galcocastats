...
const LANGUAGE_TO_SKILLS: Record<string, { category: string; weight: number }[]> = {
  TypeScript: [
    { category: 'Backend', weight: 0.6 },
    { category: 'Frontend', weight: 0.4 },
  ],
  JavaScript: [
    { category: 'Backend', weight: 0.4 },
    { category: 'Frontend', weight: 0.6 },
  ],
  Kotlin: [
    { category: 'Backend', weight: 0.8 },
    { category: 'Mobile', weight: 0.2 },
  ],
  Java: [
    { category: 'Backend', weight: 0.8 },
    { category: 'Mobile', weight: 0.2 },
  ...
  Python: [
    { category: 'Backend', weight: 0.5 },
    { category: 'DevOps', weight: 0.5 },
  ],
  Go: [
    { category: 'Backend', weight: 0.5 },
    { category: 'DevOps', weight: 0.5 },
...
function getProficiencyLevel(percentage: number): { label: string; color: string } {
  if (percentage >= 50) {
    return { label: 'Expert', color: '#10b981' }
  }
  if (percentage >= 25) {
    return { label: 'Advanced', color: '#3b82f6' }
  }
  if (percentage >= 10) {
    return { label: 'Proficient', color: '#8b5cf6' }
  }
  if (percentage >= 5) {
...
function calculateSkillsFromLanguages(
  ...
  if (devops) {
    if (devops.activeRepos >= 5) {
      skillScores['DevOps'] += 30
    } else if (devops.activeRepos >= 3) {
      skillScores['DevOps'] += 20
    }
    if (devops.codeReviews >= 30) {
      skillScores['DevOps'] += 25
    } else if (devops.codeReviews >= 10) {
      skillScores['DevOps'] += 15
    }
    if (devops.hasMultipleOrgs) {
      skillScores['DevOps'] += 20
    }
    if (devops.crossRepoWork >= 5) {
      skillScores['DevOps'] += 15
    }
  }
    ...
    <Card theme={theme} width={900} height={340}>
      <div style={{ display: 'flex', height: '100%', gap: 40 }}>
              ...
              <div key={lang.name} style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    />
                    <div
                      style={{ display: 'flex', fontSize: 15, fontWeight: 600, color: theme.text }}
                    >
                      {lang.name}
                    </div>
                  </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, alignItems: 'center' }}>
          ...
          <div
            style={{ display: 'flex', flex: 1, position: 'relative', width: 280, marginTop: 32 }}
          >
            ...
            {dataPoints.map((p, i) => {
              ...
              return (
                ...
                >
                  <div
                    style={{ display: 'flex', fontSize: 12, fontWeight: 600, color: theme.text }}
                  >
                    {p.name}
                  </div>
