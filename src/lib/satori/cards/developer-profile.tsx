...
function getMonthlyRank(stats: DeveloperStats): { rank: string; color: string; score: number } {
  ...
  if (score >= 200) {
    return { rank: 'S+', color: '#FFD700', score }
  }
  if (score >= 150) {
    return { rank: 'S', color: '#FFA500', score }
  }
  if (score >= 100) {
    return { rank: 'A+', color: '#FF6B6B', score }
  }
  if (score >= 70) {
    return { rank: 'A', color: '#4ECDC4', score }
  }
  if (score >= 50) {
    return { rank: 'B+', color: '#45B7D1', score }
  }
  if (score >= 30) {
    return { rank: 'B', color: '#96CEB4', score }
  }
  if (score >= 15) {
    return { rank: 'C', color: '#A8E6CF', score }
  }
        ...
        >
          ...
          <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center' }}>
            ...
            >
              ...
              <div style={{ display: 'flex', fontSize: 12, color: theme.accent }}>
                ~{stats.avgCommitsPerDay}/day
              </div>
            </div>
            <div
              style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}
          ...
          <div
            style={{
              display: 'flex',
              height: 1,
              background: withOpacity(theme.border, 0.19),
              margin: '14px 0',
            }}
          />
