...
export function OverviewCard({ stats, theme }: { stats: OverviewStats; theme: Theme }) {
  ...
  const mergeRateColor =
    stats.mergeRate >= 90 ? '#10b981' : stats.mergeRate >= 70 ? '#f59e0b' : '#ef4444'
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div style={{ display: 'flex', flex: 1, alignItems: 'center' }}>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div
                style={{ display: 'flex', fontSize: 38, fontWeight: 700, color: theme.sideNums }}
              >
                {stats.longestStreak}
              </div>
              <div style={{ display: 'flex', fontSize: 14, color: theme.dates }}>
            ...
            >
              ...
              <div
                style={{
                  ...
                  fontSize: 40,
                  fontWeight: 700,
                  color: theme.currStreak,
                  marginTop: 2,
                }}
              >
                {stats.currentStreak}
              </div>
              <div style={{ display: 'flex', fontSize: 12, color: theme.dates }}>weeks</div>
            </ProgressRing>
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div
                style={{ display: 'flex', fontSize: 38, fontWeight: 700, color: mergeRateColor }}
              >
                {stats.mergeRate.toFixed(0)}%
              </div>
              <div style={{ display: 'flex', fontSize: 14, color: theme.dates }}>Merge Rate</div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div
                style={{ display: 'flex', fontSize: 38, fontWeight: 700, color: theme.sideNums }}
